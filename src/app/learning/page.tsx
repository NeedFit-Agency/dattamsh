...
const createConfetti = () => {
  ...
  confetti.style.animation = `${styles.confettiFall} ${
    Math.random() * 3 + 2
  }s linear forwards`;
...
function LearningPageContent() {
  ...
  const [itemCorrectness, setItemCorrectness] = useState<{
  ...
  useEffect(() => {
    ...
    if (!selectedContent.length) {
      console.warn(
        `Content not found for Standard ${standardParam}, Chapter ${chapterParam}.`
      );
      // Fallback logic
  ...
  const checkDragDrop = () => {
    ...
    const totalPlacedInTargets =
      (dndState.naturalTarget?.length || 0) +
      (dndState.manMadeTarget?.length || 0);
  ...
  const handleContinue = () => {
    ...
    if (currentContent.type === "drag-drop") {
      ...
      const anyIncorrect = Object.values(itemCorrectness).some(
        (correct) => !correct
      );
    ...
    if (currentSlideIndex < totalSlides - 1) {
      ...
      console.log(
        "Lesson Finished! Redirecting back to chapter page for Standard/Chapter:",
        standard,
        chapter
      );
      router.push(`/standard/${standard}/chapter/${chapter}`);
  ...
  if (
    ...
    (!currentContent || currentContent.type !== "drag-drop" || dndChecked)
  ) {
    const allowFinish =
  ...
  return (
    <div>
      <div className={styles.learningContent}>
        <header className={styles.learningHeader}></header>
      ...
      {showExitConfirm && (
        ...
        >
          ...
          >
            ...
            <p>
              Your progress on this lesson might not be saved if you exit now.
            </p>
