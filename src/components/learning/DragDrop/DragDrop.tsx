...
export const DragDrop: React.FC<DragDropProps> = ({
  ...
  useEffect(() => {
    if (!feedback.show && isCheckingAnswers && !feedback.correct) {
      const timer = setTimeout(() => {
        handleReset();
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [feedback.show, isCheckingAnswers, feedback.correct]);

  const handleTouchStart = (e: React.TouchEvent, item: DragItem) => {
    if (!isTouchDevice || item.placed) return;
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    if (!isTouchDevice || !touchState.isDragging || !touchState.draggedItem)
      return;
  ...
  const handleDropItem = (item: DragItem, targetId: string) => {
    ...
    const allItemsPlaced = updatedDragItems.every(
      (dragItem) => dragItem.placed
    );
    if (allItemsPlaced) {
  ...
  const handleRemoveItem = (targetId: string, itemId: string) => {
    ...
    setDragItems((prev) =>
      prev.map((item) =>
        item.id === itemId ? { ...item, placed: false, targetId: "" } : item
      )
    );
    ...
    <div className={styles.container} ref={containerRef}>
      ...
      >
        <div
          ...
          style={{ flex: 1, marginRight: 32, minHeight: 400 }}
        >
                ...
                if (unplacedItems.length > 0) {
                  ...
                  return itemsToShow.map((item) => (
                    <motion.div
                      ...
                      onTouchStart={
                        isTouchDevice
                          ? (e: React.TouchEvent) => handleTouchStart(e, item)
                          : undefined
                      }
                  ...
                  return (
                    <div className={styles.emptyMessage}>All items placed!</div>
                  );
                }
              })()}
            </AnimatePresence>
          ...
          {targets.map((target) => (
            <div key={target.id} className={styles.targetContainer}>
              ...
              <div
                ...
                onDrop={
                  !isTouchDevice ? (e) => handleDrop(e, target.id) : undefined
                }
              >
...
export function DragDemo() {
  ...
  return (
    <div ref={constraintsRef} className={styles.demoContainer}>
      <motion.div
        drag
        ...
        whileDrag={{
          scale: 1.15,
          rotate: 10,
          boxShadow: "0 8px 24px rgba(26,162,255,0.25)",
        }}
