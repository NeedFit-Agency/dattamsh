...
export const BucketMatch: React.FC<BucketMatchProps> = ({
  ...
  const bucketAudioRef = useRef<HTMLAudioElement | null>(null);
  const [playingBucketAudio, setPlayingBucketAudio] = useState<string | null>(
    null
  );
  ...
  const allItemsMatched =
    allItemsCount > 0 && matchedItemsCount === allItemsCount;
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    const isBucketFilledCorrectly = Object.values(placedItems).includes(
      bucketId || ""
    );
  ...
  const handleDrop = (e: React.DragEvent, targetBucket: Bucket) => {
    ...
    if (isCorrect) {
      ...
      setPlacedItems((prev) => ({
        ...prev,
        [draggedItem.id]: targetBucket.id,
      }));
      ...
      const bucketName =
        targetBucket.title ||
        targetBucket.type.charAt(0).toUpperCase() + targetBucket.type.slice(1);
      ...
      <div className={styles.worksheetCard}>
        ...
        <div className={styles.matchArea}>
            ...
            {items.map((item) => (
              <div
                ...
                className={`${styles.fruitItem} ${
                  placedItems[item.id] ? styles.placed : ""
                }`}
          ...
          <div className={styles.basketsContainer}>
            {buckets.map((bucket) => {
              const isCorrectlyFilled = Object.entries(placedItems).some(
                ([itemId, bId]) =>
                  bId === bucket.id &&
                  items.find((i) => i.id === itemId)?.type === bucket.type
              );
              const itemInThisBucket = items.find(
                (i) => placedItems[i.id] === bucket.id
              );
              return (
                <div key={bucket.id} className={styles.bucketContainer}>
                  <div
                    data-bucket-id={bucket.id}
                    className={`${styles.basketDropzone} ${
                      isCorrectlyFilled ? styles.correct : ""
                    } ${
                      feedback.bucketId === bucket.id &&
                  ...
                  >
                    ...
                    {itemInThisBucket && isCorrectlyFilled && (
                      <div className={styles.droppedFruit}>
                        {getFruitSvg(
                          itemInThisBucket.type,
                          itemInThisBucket.imageUrl
                        )}
                      </div>
                    )}
                  </div>
                  <div className={styles.bucketLabelContainer}>
                    {bucket.audioSrc ? (
                      <Button
                        ...
                        onClick={() => {
                          ...
                          audio.addEventListener("error", () => {
                            console.error("Bucket audio play failed");
                            setPlayingBucketAudio(null);
                            setIsAnyAudioPlaying(false);
                          });
                      ...
                      <TTS
                        text={
                          bucket.title ||
                          bucket.type.charAt(0).toUpperCase() +
                            bucket.type.slice(1)
                        }
        ...
        {/* Feedback message - positioned as a center screen popup */}
